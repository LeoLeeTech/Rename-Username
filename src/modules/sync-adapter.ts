...
const SCRIPT_NAME = '[Rename Username Extension Sync Adapter]'
...
async function checkUserscriptAvailable(): Promise<boolean> {
  try {
    ...
    console.warn('[Rename Username] Userscript may be disabled:', error)
    return false
  }
}
...
async function initExtensionId(): Promise<void> {
  ...
  MY_EXTENSION_NAME = `Rename Username ${type}${tag}`
