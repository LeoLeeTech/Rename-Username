...
function handleHttpRequestExtension(
  ...
  console.log(`[Rename Username Extension] Processing HTTP request: ${method} ${url}`)
  ...
  chrome.runtime
    ...
    .catch((error: unknown) => {
      console.error(
        '[Rename Username Extension] Error communicating with background script:',
        error
      )
...
function handleHttpRequestUserscript(
  ...
  void gmRequest({
    ...
    onload(response) {
      console.log(
        `[Rename Username Extension] HTTP request successful: ${response.status}`
      )
    ...
    onerror(error) {
      console.error(`[Rename Username Extension] HTTP request failed:`, error)
      sendHttpError(
        id,
    ...
    ontimeout() {
      console.error(`[Rename Username Extension] HTTP request timeout`)
...
function handlePing(message: PingMessage, event: MessageEvent): void {
  console.log('[Rename Username Extension] Received ping, sending pong')
...
function messageListener(event: MessageEvent): void {
  ...
  try {
    ...
    console.log(`[Rename Username Extension] Received message:`, message.type)
    ...
    switch (message.type) {
      ...
      default: {
        ...
        console.log(`[Rename Username Extension] Unknown message type: ${message.type}`)
      }
    }
  } catch (error) {
    console.error('[Rename Username Extension] Error handling message:', error)
...
export function setupWebappBridge(): void {
  ...
  console.log('[Rename Username Extension] ready for HTTP proxy requests')
}
...
//     id: `ready-${Date.now()}`,
//     payload: {
//       name: 'Rename Username HTTP Proxy Extension',
//       version: '1.0.0',
//     },
//   }
